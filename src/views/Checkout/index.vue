...
const activeIndex = ref(0) // 控制地址框集火项
...
const openDialog = () => {
  const index = checkInfo.value.userAddresses.findIndex((item) => item.id === curAddress.value.id)
...
const createOrder = async () => {
  const data = {
    ...
    payChannel: 1, //支付渠道
    buyerMessage: '', //买家备注留言
    goods: checkInfo.value.goods.map((item) => ({ skuId: item.skuId, count: item.count })), //订单内商品集合
